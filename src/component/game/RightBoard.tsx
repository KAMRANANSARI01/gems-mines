...
const RightBoard: React.FC<{
      ...
      history: any[];
    }> = ({ boxes, revealed, gameStatus, clickedIndices, handleReveal, history }) => {
      return (
        <main className="w-full flex items-center justify-center">
          <div className="right_part min-h-[calc(100vh-114px)] rounded-br-xl rounded-tr-xl  p-6 shadow-xl h-full flex flex-col w-full items-center justify-center">
              ...
              {history.map((h) => (
                <div
                  ...
                  className={`flex-shrink-0 min-w-[40px] px-2 py-1 rounded-md text-xs font-medium flex items-center justify-center ${
                    h.kind === "lose" ? "bg-gray-700 text-gray-200" : "bg-green-600 text-white"
                  }`}
            ...
            <div className=" bg-[#323738] rounded-xl flex justify-center items-center p-4 relative">
              <img className="w-full max-w-[150px] absolute top-[-31px] right-[40px] " src={bird} alt="" />
              <div
              ...
              >
                {boxes.map((box, i) => (
                  <div
                    ...
                    className={`w-full max-w-[80px] aspect-square rounded-lg flex items-center justify-center select-none ${gameStatus==="idle"?"cursor-not-allowed":"cursor-pointer"} transition-transform transform shadow-md  ${
                      revealed[i]
                        ? box === "bomb"
